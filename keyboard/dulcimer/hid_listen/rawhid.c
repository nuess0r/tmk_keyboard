 ...
 * paul@pjrc.com
 * modification for dulcimer TMK: nussgipfel@brain4free.org
 *
...
// report descriptor and just hardcodes a signature for the dulcimer
// TMK firmware.  Lacking from all platforms are functions to
...
rawhid_t * rawhid_open_only1(int vid, int pid, int usage_page, int usage)
{
	...
	const unsigned char signature[]={0x05,0x01,0x09,0x06,0xa1};
	...
	for (i=0; i<HIDRAW_MAX_DEVICES; i++) {
		...
		if (r < 0) continue;
		printf("Signature of device %i is: %#x, %#x, %#x, %#x, %#x\n",i, desc->value[0], desc->value[1], desc->value[2], desc->value[3], desc->value[4]);
